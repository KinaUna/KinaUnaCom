...
namespace KinaUnaProgenyApi.Controllers
{
    ...
    public class TimeLineController : ControllerBase
    {
        ...
        public async Task<IActionResult> Progeny(int id, [FromQuery] int accessLevel = 5)
        {
            ...
            {
                List<TimeLineItem> timeLineList = await _dataService.GetTimeLineList(id); 
        ...
        public async Task<IActionResult> ProgenyYearAgo(int id, int accessLevel = 5)
        {
            ...
            {
                List<TimeLineItem> timeLineList = await _dataService.GetTimeLineList(id);
                timeLineList = timeLineList.Where(t => t.AccessLevel >= accessLevel && t.ProgenyTime.Year < DateTime.UtcNow.Year && t.ProgenyTime.Month == DateTime.UtcNow.Month && t.ProgenyTime.Day == DateTime.UtcNow.Day).OrderBy(t => t.ProgenyTime).ToList();
        ...
        {
            TimeLineItem result = await _dataService.GetTimeLineItemByItemId(itemId, itemType); 
        ...
        {
            Progeny prog = await _dataService.GetProgeny(id);
            if (prog != null)
            {
                string userEmail = User.GetEmail() ?? Constants.DefaultUserEmail;
